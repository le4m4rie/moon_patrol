...
/**
 * This class represents the position of the game objects. In the upper left corner of the
 * {@link GameView} window, the position is (0, 0). The first digit represents the x-axis and the second
 * one the y-axis. In the lower right corner, the position corresponds to (1280, 720).
 *
 * @see GameView
 */
public class Position implements Comparable<Position> {
    ...
    public Position(Position otherPosition) {
        this(otherPosition.getX(), otherPosition.y);
    }
    ...
    /**
     * Calculates the distance between the position of an object and the position of a given other object.
     *
     * @param otherPosition Another position.
     * @return the total distance to another position.
     */
    public double distance(Position otherPosition) {
        double xDistance = (x - otherPosition.x);
        double yDistance = (y - otherPosition.y);
    ...
    public boolean similarTo(Position otherPosition) {
        return Math.round(x) == Math.round(otherPosition.x) && Math.round(y) == Math.round(otherPosition.y);
    }
    ...
    public int compareTo(Position other) {
        ...
        return Double.compare(distance(zeroPosition), other.distance(zeroPosition));
    }
